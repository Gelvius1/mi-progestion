    ...
    public function read(...$params) {
        ...
        if ($result) {
            if ($result->num_rows > 0) {
                    ...
                    switch ($params[0]) {
                        case 'mer':
                            switch ($params[2]) {
                                ...
                                case 'type':
								case 'unasigned':
									$count = isset($this->response['data']) ? count($this->response['data']) : 0;

									$this->response['message'] = ($params[2] == 'onload') ? 'Se han seleccionado $count datos por carga.' :
										(($params[2] == 'unasigned') ? 'Se seleccionaron $count tareas sín intervalo.' :
										(($params[2] == 'type') ? (
											($params[3] == 0) ? 'Se han seleccionado $count datos ocasionales.' :
											(($params[3] == 1) ? 'Se han seleccionado $count datos periódicos.' :
											(($params[3] == '*') ? 'Se han seleccionado $count datos.' :
											'Se han seleccionado $count datos desconocidos'))
										) : 'Se han seleccionado $count datos por tipo.'));
						...
						case 'tng':
							if ($params[2] == 'onload') {
								$count = isset($this->response['data']) ? count($this->response['data']) : 0;
								$this->response['message'] = "Se han seleccionado $count intervalos por carga.";
								$this->response['data'][] = [
									'id' => $row['id'],
									'name' => $row['name'],
                ...
                switch ($params[0]) {
                    case 'mer':
                        if ($params[2] == 'onload' || $params[2] == 'type') {
